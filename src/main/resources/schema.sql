...
CREATE TABLE IF NOT EXISTS `user_identity` (
    ...
    `evaluation_completed` TINYINT NOT NULL DEFAULT 0 COMMENT '是否完成测评（0否 1是）',
    `evaluation_score` INT COMMENT '测评总分',
    `evaluation_level` VARCHAR(20) COMMENT '测评等级：R1/R2/R3/R4/R5',
    `evaluation_completed_at` DATETIME COMMENT '测评完成时间',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    ...
    INDEX idx_evaluation_completed (`evaluation_completed`),
...
CREATE TABLE IF NOT EXISTS `loan_product` (
    ...
    `product_type` VARCHAR(50) NOT NULL COMMENT '产品分类（短途游贷/毕业旅行贷/研学游贷/跨省游贷）',
...
CREATE TABLE IF NOT EXISTS `loan_application` (
    ...
    `apply_term` INT NOT NULL COMMENT '申请期限（月）',
    `purpose` VARCHAR(500) COMMENT '贷款用途',
    `status` VARCHAR(20) NOT NULL DEFAULT 'PENDING' COMMENT '申请状态：PENDING-待审核, APPROVED-已批准, REJECTED-已拒绝, DISBURSED-已发放, COMPLETED-已完成',
    `apply_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '申请时间',
    `approve_time` DATETIME COMMENT '审批时间',
    `reject_reason` VARCHAR(500) COMMENT '拒绝原因',
    `approver_id` BIGINT COMMENT '审批人ID',
    `loan_amount` DECIMAL(10, 2) COMMENT '实际放款金额',
